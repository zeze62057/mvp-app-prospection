...
| `agent-infographe` | Génère l'affiche visuelle d'un post LinkedIn déjà rédigé et l'attache à sa ligne dans la base Notion "Posts rédigés" | Juste après `agent-linkedin` (une invocation par post), ou à la main avec l'URL ou l'id d'une ligne | `.claude/agents/agent-infographe.md` |

## Chaînage

`agent-linkedin` rédige les posts, puis `agent-infographe` produit l'affiche de chacun.
