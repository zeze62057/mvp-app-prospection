...
Pour Chatllow, une recherche People Database viserait des décideurs (poste : direction générale, direction transformation digitale) dans des entreprises d'une taille correspondant au CAC40 ou à des grands comptes équivalents, plutôt qu'une liste générique de "décideurs" sans filtre de secteur ou de taille.
...
Un Tracking Sheet centralise le statut de chaque prospect en cours de conversation, pour ne jamais perdre le fil d'un échange. Un CRM va plus loin en structurant tout le cycle de vie du prospect (Kora, l'application déjà construite dans ce parcours, applique exactement ce principe pour le marketing de réseau).
...
4. Quel outil déjà construit dans ce parcours illustre le principe d'un CRM appliqué à un besoin de prospection ?

### Réflexion (synthèse de fin de section)
5. Rédige les grandes lignes (pas le texte complet) d'une séquence de 3 messages pour une campagne Chatllow ou Longrich, en identifiant ce que chaque message apporte de nouveau.

### Éléments de correction (réservé à l'enseignant)
- Q1 : sans un bon prompt (gabarit à 4 éléments), l'IA produit un texte générique peu exploitable malgré la rapidité de génération
- Q2 : ça évite un travail manuel de nettoyage après coup et réduit les erreurs, la qualité de la base dépend directement de la précision des filtres
- Q3 : le principe MVP avant V1, valider que la séquence fonctionne réellement avant d'exposer toute la base à un message non éprouvé
- Q4 : Kora, l'application de prospection MLM déjà construite dans ce parcours
- Q5 : pas de réponse unique, évaluer que chaque message apporte réellement quelque chose de nouveau, pas une simple relance répétitive
