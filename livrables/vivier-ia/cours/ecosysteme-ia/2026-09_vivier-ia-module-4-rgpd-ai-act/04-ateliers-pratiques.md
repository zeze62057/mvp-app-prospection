...
> Cinq ateliers qui appliquent concrètement les principes des sections précédentes, sur des cas réels transposables à Chatllow ou à Vivier IA.
...
2. Vérifier que cette clé vit uniquement dans une variable d'environnement (fichier `.env.local` non commité, ou équivalent) et jamais dans un fichier suivi par Git, exactement la même discipline que pour les clés Supabase ou Chariow déjà pratiquée dans ce parcours.
...
6. Choisis un des 5 ateliers et explique en une phrase comment tu l'appliquerais concrètement sur un projet réel de Chatllow.
...
- Q1 : envoyer des données client à un modèle qui les utiliserait pour son entraînement ou les conserverait indéfiniment serait une violation de conformité difficile à rattraper après coup
- Q2 : remplacer une donnée identifiante par un identifiant générique avant l'envoi au modèle, puis remapper le résultat vers la vraie identité seulement après, dans une étape isolée
- Q3 : photo, date de naissance précise, mention d'origine ou de situation familiale non pertinente (3 au choix)
- Q4 : authentification dédiée par projet, stockage jamais en clair (variable d'environnement), accès restreint aux identifiants eux-mêmes
- Q5 : extraire l'information nécessaire puis supprimer le document source, sauf raison précise de conservation avec accès restreint et durée définie
- Q6/Q7 : pas de réponse unique, évaluer la pertinence et l'appropriation personnelle de la réponse
