...
import { creerCreneauRdv } from "@/app/admin/actions";

const etatInitial = { erreur: null, succes: false };

export function FormulaireCreneauRdv({ espaces }: { espaces: { id: string; nom: string }[] }) {
  const [etat, action] = useActionState(creerCreneauRdv, etatInitial);
  ...
  return (
    ...
    >
      ...
      {etat.succes && <p className="w-full text-xs text-[var(--sarcelle)]">Creneau cree.</p>}
    </form>
  );
}
