...
- **Lots B, C et D construits le 2026-09-20 (migrations 0027 à 0030)** :
  - **Correctif de sécurité (0027)** : un membre pouvait se donner le rôle admin (et des points), s'auto-approuver dans la communauté gratuite, ou créer un paiement déjà confirmé. Un membre ne modifie plus que son pseudo, et ses insertions sur `adhesions` et `paiements` sont forcées "en attente". Aucune trace d'abus dans la base avant le correctif
  - **B, photo de profil (0028)** : upload par le membre depuis « Mon profil », bucket privé `avatars` (2 Mo, JPG, PNG ou WebP, signature vérifiée), liens temporaires, affichée dans le fil, les commentaires et la barre d'écriture
  - **C, notifications (0029)** : like, commentaire ou message reçu, créés par des triggers (un membre ne peut pas en fabriquer). Un like retiré puis remis ne notifie qu'une fois, et plusieurs messages non lus d'un même expéditeur forment une seule notification. Page dédiée, « tout marquer comme lu »
  - **D, messagerie privée (0030)** : entre deux membres du même espace seulement, lisible uniquement par eux deux (aucun chemin de lecture pour l'admin), texte seul, 2000 caractères. Ni blocage ni signalement dans cette version. On ouvre une conversation depuis « ✉ Écrire à… » sous un post ou un commentaire, il n'y a pas de recherche de membre
  - **Barre de navigation** : Accueil, Messages, Notifications (badge), photo de profil. Fixée en bas sur mobile, pastille flottante sur ordinateur. Le badge est actualisé toutes les 30 secondes, il n'y a pas de temps réel
  - Un incident réseau vers Supabase n'est plus confondu avec une déconnexion (message dédié, page d'erreur réessayable au lieu d'un renvoi vers la connexion)
  - Vérifié dans le navigateur avec des comptes de test jetables (supprimés ensuite) : photo, like et commentaire qui notifient, message avec badge, réponse, et 11 tentatives d'un non-membre sur l'API réelle toutes refusées
- **Compteur, temps réel, recherche, blocage, signalement (2026-09-20, migration 0031)** :
  - **Compteur de membres et classement** : ils étaient faux dès qu'il y avait plusieurs membres (`adhesions` n'est lisible que pour ses propres lignes). Une fonction `stats_communaute` les calcule, ainsi que la liste des élèves de la zone payante. Elle ne renvoie que ce qu'un co-membre voit déjà
  - **Temps réel (Supabase Realtime)** : `notifications` et `messages` sont dans la publication. Le badge et les messages arrivent sans recharger la page (mesuré : 2,9 s et 4,1 s). Le client navigateur ne transmet pas la session au temps réel tant qu'on ne le lui dit pas : sans `clientTempsReel`, les abonnements sont acceptés mais restent muets. Un sondage de secours toutes les 2 minutes reste en place
  - **Recherche de membre** : membres de l'espace seulement, hors soi-même et hors blocages, 20 résultats. Le texte saisi n'est jamais interprété comme un motif (`%` et `_` ne sont pas des jokers)
