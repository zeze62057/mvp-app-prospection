...
const ETAPES: { t: string; d: string; puces?: string[] }[] = [
  ...
  { t: "Choisir les bons outils", d: "Selon vos besoins, vous pouvez utiliser des solutions comme :", puces: ["Chatbots IA (ex. : ChatGPT, Claude, Copilot…)", "Plateformes de service client (ex. : Zendesk, Intercom, etc.)"] },
  ...
  return (
    <section className="flex flex-col rounded-2xl border border-[var(--ligne)] bg-[var(--fond-carte)]">
      <div className="flex flex-col gap-5 p-5 sm:p-6">
        ...
        <div className="flex items-start justify-end gap-3">
          <div className="max-w-[78%]">
            <p className="mb-1 text-right font-[family-name:var(--font-mono)] text-[10.5px] text-[var(--texte-mute)]">10:24</p>
            <div className="rounded-2xl rounded-tr-md bg-[var(--indigo-soft)] px-4 py-3 text-[13.5px] leading-relaxed">{QUESTION}</div>
          </div>
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="min-w-0 max-w-[88%]">
            <p className="mb-1 font-[family-name:var(--font-mono)] text-[10.5px] text-[var(--texte-mute)]"><b className="font-semibold text-[var(--texte)]">Chatllow IA</b> · 10:24</p>
              ...
              <ol className="my-3 flex list-none flex-col gap-3 pl-0 [counter-reset:etape]">
                {ETAPES.map((e) => (
                  ...
                  >
                    ...
                    <span className="block text-[13px] text-[var(--texte-mute)]">{e.d}</span>
                    {e.puces && (
                      <ul className="mt-1 list-disc pl-5 text-[13px] text-[var(--texte-mute)]">
                        {e.puces.map((p) => (
                          <li key={p}>{p}</li>
                        ))}
                      </ul>
            ...
            <div className="mt-2 flex flex-wrap gap-2" aria-hidden>
              {([["Copier", "copier"], ["Télécharger la synthèse", "telecharger"], ["Voir le plan d'action", "liste"]] as const).map(([b, ic]) => (
                <span key={b} className="inline-flex items-center gap-1.5 rounded-full border border-[var(--ligne)] px-3.5 py-1 text-[11.5px] font-semibold text-[var(--texte-mute)] opacity-70">
                  <Icone nom={ic} className="h-3.5 w-3.5" />
                  {b}
                </span>
              ))}
            </div>
      ...
      <div className="flex items-center gap-3 border-t border-[var(--ligne)] p-3.5">
        <span aria-hidden className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-[var(--indigo-soft)] text-[oklch(45%_0.19_250)] opacity-60">
          <Icone nom="plus" />
        </span>
        <input
          disabled
        ...
        />
        <Icone nom="trombone" className="h-[18px] w-[18px] text-[var(--texte-mute)] opacity-50" />
        <Icone nom="etincelle" className="h-[18px] w-[18px] text-[var(--texte-mute)] opacity-50" />
