...
5. **Construction, tâche par tâche** — le gabarit à 4 éléments (Contexte, Objectif, Périmètre, Autonomie) pour cadrer chaque tâche, puis le cycle Plan, Execute, Validate pour l'exécuter. Si une automatisation n8n a été décidée à l'étape 1, elle se construit ici séparément du produit : tester chaque partie isolément (par exemple, un webhook appelé directement, sans passer par le vrai bouton de l'interface) avant de les connecter entre elles. Dès qu'une tâche implique une manipulation technique réelle sur l'infrastructure ou les comptes de Zézé (installation sur un serveur, configuration d'un service tiers, paramétrage d'un compte), le signaler explicitement avant de commencer et procéder étape par étape en direct avec lui, en vérifiant chaque étape avant de passer à la suivante, plutôt que de dérouler une liste complète d'instructions à exécuter seul de son côté.
...
- Une manipulation technique réelle sur l'infrastructure ou les comptes de Zézé (serveur, service tiers, compte) se signale et se fait en direct, étape par étape, avec vérification à chaque étape, jamais comme une liste d'instructions livrée d'un bloc à exécuter seul
