...
Instruction floue : "améliore le générateur d'audit IA". Instruction complète : "Dans le projet `generateur-audit-ia-hotellerie`, l'objectif est que le rapport final inclue une section chiffrée de ROI estimé. Le périmètre est uniquement le template de rapport, ne touche pas au formulaire de collecte de données. Propose-moi la méthode de calcul avant de l'implémenter, je veux la valider." La seconde donne à l'agent tout ce qu'un collègue qui découvre le projet aujourd'hui aurait besoin de savoir, la première le laisse deviner.
