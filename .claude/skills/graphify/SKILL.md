...
```bash
# PATCH LOCAL (workspace de Zezé) : interpreteur fixe dans un environnement isole, AUCUNE installation automatique.
# Le skill d'origine tentait `pip install graphifyy` dans le Python global. Ici, si l'outil manque, on s'arrete.
PYTHON="$LOCALAPPDATA/graphify-venv/Scripts/python.exe"
if ! "$PYTHON" -c "import graphify" 2>/dev/null; then
    echo "graphify introuvable dans %LOCALAPPDATA%/graphify-venv : arret, aucune installation automatique."
    exit 1
fi
# Pour les commandes `graphify ...` nues plus bas, utiliser : "$LOCALAPPDATA/graphify-venv/Scripts/graphify.exe" ...
