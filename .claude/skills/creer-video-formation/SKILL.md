...
**Copie sur le Bureau (demande de Zézé, 2026-09-21).** Une fois le PowerPoint vérifié, copiez-le sur le Bureau de Zézé : `Bureau\Diapositives Vivier IA\Module <N> - Section <M> - <Nom de la section>\Chapitre <K>.pptx` (chemin du Bureau par `[Environment]::GetFolderPath('Desktop')`). Créez le dossier de la section s'il n'existe pas, et écrasez la copie du même chapitre après une reconstruction. C'est une copie : la source reste `diapositives.json` dans le projet. Si le fichier est ouvert dans PowerPoint et que la copie échoue, dites-le à Zézé.
